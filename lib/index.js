...
function phantomas(url, options, callback) {
	...
	proc.on('close', function(code) {
		...
		try {
			...
			// set non-zero exit code when JSON parsing failed
			// thiscan be caused by segfaulting PhantomJS (issue #382)
			if (code <= 250) {
				code = 251; // EXIT_LOAD_ABORTED

				errStream.push('phantomas: (' + code + ') loading aborted - no results were generated\n');
			}
		}
